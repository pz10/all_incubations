folder_out <-paste0(output_path, "/presentation_plots/boxplot_byvolume")
...
events.file <- paste0(input_path_RAW, "/_extra/rain_events_time.dat")
...
cum.A[, incubation:= "A"]
#
cum.A[, treatment1:= treatment]
myvolume <- events[incubation=="A", list(treatment, volume_factor)]
setkey(cum.A, treatment1)
setkey(myvolume, treatment)
cum.A <- cum.A[myvolume]
cum.A[,treatment1:=NULL]
#############################
### B incubation
cum.B <- copy(data)
cum.B[, NO:= bNO]
...
#
cum.B[, treatment1:= treatment]
myvolume <- events[incubation=="B", list(treatment, volume_factor)]
setkey(cum.B, treatment1)
setkey(myvolume, treatment)
cum.B <- cum.B[myvolume]
cum.B[,treatment1:=NULL]
#############################
### C incubation
cum.C <- copy(data)
cum.C[, NO:= cNO]
...
#
cum.C[, treatment1:= treatment]
myvolume <- events[incubation=="C", list(treatment, volume_factor)]
setkey(cum.C, treatment1)
setkey(myvolume, treatment)
cum.C <- cum.C[myvolume]
cum.C[,treatment1:=NULL]
#############################
### D incubation
cum.D <- copy(data)
cum.D[, NO:= dNO]
...
#
cum.D[, treatment1:= treatment]
myvolume <- events[incubation=="D", list(treatment, volume_factor)]
setkey(cum.D, treatment1)
setkey(myvolume, treatment)
cum.D <- cum.D[myvolume]
cum.D[,treatment1:=NULL]

### all incubation binding
data <- rbind(cum.A, cum.B, cum.C, cum.D)
data[,NO:= NO/volume_factor]
################################################################################
################################################################################
...
### tillage boxplot
# by treatment
myplot <- paste0(folder_out, "/NO_boxplot_tillage_bytreatment.png")
...
myplot <- paste0(folder_out, "/NO_boxplot_tillage.png")
...
# by treatment
myplot <- paste0(folder_out, "/NO_boxplot_precipitation_bytreatment.png")
png(filename = myplot, width = 1600, height = 1200, units = "px")
...
# all treatments together
myplot <- paste0(folder_out, "/NO_boxplot_precipitation.png")
png(filename = myplot, width = 320, height = 1200, units = "px")
...
### fertilizer boxplot
# by treatment
myplot <- paste0(folder_out, "/NO_boxplot_fertilizer_bytreatment.png")
png(filename = myplot, width = 1600, height = 1200, units = "px")
...
# all treatments together
myplot <- paste0(folder_out, "/NO_boxplot_fertilizer.png")
png(filename = myplot, width = 320, height = 1200, units = "px")

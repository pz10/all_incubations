folder.out <-paste0(output_path, "/cum_fluxes_14/peak_delay_1st_event")
dir.create(folder.out)

myfolder <- paste0(output_path, "/cum_fluxes_full")
myfile <- paste0(myfolder, "/1st_event_cum_fluxes.dat")
data <- fread(myfile)
...
data[,last.NO:= tail(NO,1), by= .(incubation, fertilization, precipitation, tillage)]
data[,last.N2O:= tail(N2O,1), by= .(incubation, fertilization, precipitation, tillage)]
data[,last.CO2:= tail(CO2,1), by= .(incubation, fertilization, precipitation, tillage)]

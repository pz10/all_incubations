...
heading <- c("days2ndEvent", "treatment", "fertilizer", "precipitation", "tillage",
...
# write common frame to output object: days2ndEvent and treatments
myfluxes[,days2ndEvent:= rep(mytimes, nrow(treatments))]
...
for(i in 1:4){
        ...
        for(j in mytreatments){
                ...
                mydata <- get(myfile)[!is.na(NO)
                                      & days2ndEvent >= 0
                                      & days2ndEvent <= (lastday+1)
                                      & treatment==j
                                      , list(days2ndEvent, treatment, NO)]
                # insert origin (days2ndEvent == 0), with value as last measurement before origin
        ...
        mydata[,NO:= NO+offsetNO] # later is subtracted again
                ...
                mycum <- approx(x = mydata$days2ndEvent, y = mydata$cum, xout = mytimes, method = "linear", rule = 2:2)
        ...
        print(paste0(Sys.time(), "  ", LETTERS[i], "_incubation NO-cumulative values have been calculated (2nd rain event)"))
...
for(i in 1:4){
        ...
        for(j in mytreatments){
                ...
                mydata <- get(myfile)[!is.na(N2O)
                                      ...
                                      & days2ndEvent <= (lastday+1)
                                      & treatment==j
                                      , list(days2ndEvent, treatment, N2O)]
                # insert origin (days2ndEvent == 0), with value as last measurement before origin
        ...
        print(paste0(Sys.time(), "  ", LETTERS[i], "_incubation N2O-cumulative values have been calculated (2nd rain event)"))
...
for(i in 1:4){
        ...
        for(j in mytreatments){
                ...
                mydata <- get(myfile)[!is.na(CO2)
                                      ...
                                      & days2ndEvent <= (lastday+1)
                                      & treatment==j
                                      , list(days2ndEvent, treatment, CO2)]
                # insert origin (days2ndEvent == 0), with value as last measurement before origin
        ...
        print(paste0(Sys.time(), "  ", LETTERS[i], "_incubation CO2-cumulative values have been calculated (2nd rain event)"))
...
for(i in 1:4){
        ...
        for(j in mytreatments){
                ...
                mydata <- get(myfile)[!is.na(CH4)
                                      ...
                                      & days2ndEvent <= (lastday+1)
                                      & treatment==j
                                      , list(days2ndEvent, treatment, CH4)]
                # insert origin (days2ndEvent == 0), with value as last measurement before origin
        ...
        print(paste0(Sys.time(), "  ", LETTERS[i], "_incubation CH4-cumulative values have been calculated (2nd rain event)"))
...
# format
no.format <- c("days2ndEvent", "treatment", "fertilizer", "precipitation", "tillage")
...
write.table(myfluxes, file= cum.flux.2nd, row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")

print(  paste0(Sys.time(), " 2nd rain event cumulative fluxes file was written to:  ")  )
print(cum.flux.2nd)
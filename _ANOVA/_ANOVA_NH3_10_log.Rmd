...
--> log(NH3+1) transformation is not better solution than no transformation. A non-parametric solution might be considered.
</span>
...
```{r, echo=F}
# replace strings:
# 1. "log(NH3) ~" --> "transformation ~"
# 2. "[,log(NH3)" --> "[,transformation"
```

```{r}
fit <- lm(log(NH3+1) ~ fertilization + precipitation + tillage, data=mydata)
fit.aov <- aov(log(NH3+1) ~ fertilization + precipitation + tillage, data=mydata)
fit.aov.int <- aov(log(NH3+1) ~ (fertilization + precipitation + tillage)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lm(log(NH3+1) ~ precipitation + tillage, data=mydata)
fit2 <- lm(log(NH3+1) ~ fertilization + tillage, data=mydata)
...
# visualization
                # library(HH)
interaction2wt(log(NH3+1) ~ (fertilization + precipitation + tillage)^2, data=mydata, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# # outlier removed
# myfit <- lm(log(NH3+1) ~ fertilization + precipitation + tillage, data=mydata[-c(68)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
```
...
### linearity
################################################################################
# boxTidwell(log(NH3+1) ~ fertilization + precipitation + tillage, data=mydata)
...
```{r}
library(leaps)
leaps <-regsubsets(log(NH3+1) ~ fertilization + precipitation + tillage, data=mydata, nbest=4)

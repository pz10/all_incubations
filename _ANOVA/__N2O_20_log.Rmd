---
title: "N2O differences. Approaches: Non-parametric and parametric (log transform) Interactionj considered"
...
par(mfrow=c(1,4))
with(data, boxplot(log(N2O) ~ fertilization, main = "fertilization"))
with(data, boxplot(log(N2O) ~ precipitation, main = "precipitation"))
with(data, boxplot(log(N2O) ~ tillage, main = "tillage"))
with(data, boxplot(log(N2O) ~ incubation, main = "incubation"))
...
```{r}
fit <- lm(log(N2O) ~ fertilization + precipitation + tillage, data=data)
fit.aov <- aov(log(N2O) ~ fertilization + precipitation + tillage, data=data)
fit.aov.int <- aov(log(N2O) ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(log(N2O) ~  precipitation + tillage, data=data)
fit2 <- lm(log(N2O) ~  fertilization + tillage, data=data)
fit3 <- lm(log(N2O) ~  fertilization + precipitation, data=data)

fit.int <- lm(log(N2O) ~ ( fertilization + precipitation + tillage)^2, data=data)

# fit <- lm(log(N2O) ~ fertilization + precipitation + tillage + fertilization:precipitation + fertilization:tillage, data=data)
# fit.aov <- fit.aov <- aov(log(N2O) ~ fertilization + precipitation + tillage + fertilization:precipitation + fertilization:tillage, data=data)
...
```{r}
fit.p <- lmp(log(N2O) ~  fertilization + precipitation + tillage, data=data)
fit.aovp <- aovp(log(N2O) ~  fertilization + precipitation + tillage, data=data)
fit.aovp.int <- aovp(log(N2O) ~ ( fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1.p <- lmp(log(N2O) ~  precipitation + tillage, data=data)
fit2.p <- lmp(log(N2O) ~  fertilization + tillage, data=data)
fit3.p <- lmp(log(N2O) ~  fertilization + precipitation, data=data)

fit.int.p <- lmp(log(N2O) ~ ( fertilization + precipitation + tillage)^2, data=data)
...
# 
# # outlier removed
# myfit <- lm(log(N2O) ~ fertilization + precipitation + tillage, data=data[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(log(N2O) ~ fertilization + precipitation + tillage, data=data[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(log(N2O) ~ fertilization + precipitation + tillage, data=data[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(log(N2O) ~ fertilization + precipitation + tillage, data=data[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(log(N2O) ~ fertilization + precipitation + tillage, data=data[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
...
summary(powerTransform(data[,log(N2O)]+1))
```
...
### linearity
################################################################################
# boxTidwell(log(N2O) ~ fertilization + precipitation + tillage, data=data)
...
```{r}
library(leaps)
leaps <-regsubsets(log(N2O) ~ ( fertilization + precipitation + tillage), data=data, nbest=4)
...
```{r, echo=F}
# fert
results.fert <- W.MW.oneway(log(N2O) ~ fertilization, data, n=my.n)
summary(results.fert)
plot(results.fert)

# precipitation
results.rain <- W.MW.oneway(log(N2O) ~ precipitation, data, n=my.n)
summary(results.rain)
plot(results.rain)

# tillage
results.tillage <- W.MW.oneway(log(N2O) ~ tillage, data, n=my.n)
...
```{r, echo=F}
# # fert
# results.fert <- PToneway(log(N2O) ~ fertilization, data, n=my.n)
# summary.oneway(results.fert)
# 
# # precipitation
# results.rain <- PToneway(log(N2O) ~ precipitation, data, n=my.n)
# summary.oneway(results.rain)
# 
# # tillage
# results.till <- PToneway(log(N2O) ~ tillage, data, n=my.n)

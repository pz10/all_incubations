---
title: "N2O differences (permutation test) Without transformation. Non-parametric approach"
...
```{r, results='hide'}
fit <- lmp(N2O ~ fertilization + precipitation + tillage, data=data)
fit.aov <- aovp(N2O ~ fertilization + precipitation + tillage, data=data)
fit.aov.int <- aovp(N2O ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lmp(N2O ~ precipitation + tillage, data=data)
fit2 <- lmp(N2O ~ fertilization + tillage, data=data)
fit3 <- lmp(N2O ~ fertilization + precipitation, data=data)
```
...
# visualization
                # library(HH)
interaction2wt(N2O ~ (fertilization + precipitation + tillage)^2, data=data, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# 
# # outlier removed
# myfit <- lmp(N2O ~ fertilization + precipitation + tillage, data=data[-c(68)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
```
...
outlierTest(fit)
next.outlier <- lmp(N2O ~ fertilization + precipitation + tillage, data=data[-c(68)]) # only 17 is an outlier
...
summary(powerTransform(data[,N2O]))
```

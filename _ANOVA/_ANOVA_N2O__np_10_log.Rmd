---
title: "N2O differences (permutation test). log(N2O) transformation"
...
--> log(N2O) transformation model fulfill assumptions
</span>
...
```{r, results='hide'}
fit <- lmp(log(N2O) ~ fertilization + precipitation + tillage, data=data)
fit.aov <- aovp(log(N2O) ~ fertilization + precipitation + tillage, data=data)
fit.aov.int <- aovp(log(N2O) ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lmp(log(N2O) ~ precipitation + tillage, data=data)
fit2 <- lmp(log(N2O) ~ fertilization + tillage, data=data)
fit3 <- lmp(log(N2O) ~ fertilization + precipitation, data=data)
```
...
# visualization
                # library(HH)
interaction2wt(log(N2O) ~ (fertilization + precipitation + tillage)^2, data=data, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# 
# # outlier removed
# # myfit <- lmp(log(N2O) ~ fertilization + precipitation + tillage, data=data[-c(44)])
# # gvmodel <- gvlma(myfit)
# # summary(gvmodel)
```
...
outlierTest(fit)
next.outlier <- lmp(log(N2O) ~ fertilization + precipitation + tillage, data=data[-c(44)])
outlierTest(next.outlier)
next.outlier <- lmp(log(N2O) ~ fertilization + precipitation + tillage, data=data[-c(44,49+1)])
...
summary(powerTransform(data[,log(N2O)]))
```

<span style="color:red">
--> null hypothesis λ = 1 cannot be rejected: no need for transformation
</span>

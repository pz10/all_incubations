---
title: "NH3 differences. log(NH3 +100) transformation. Tillage not considered in the OLS model"
...
```{r, echo=F}
# replace strings:
# 1. "log(NH3+100) ~" --> "transformation ~"
# 2. "[,log(NH3+100)]" --> "[,transformation]"
```

```{r}
fit <- lm(log(NH3+100) ~ fertilization + precipitation, data=mydata)
fit.aov <- aov(log(NH3+100) ~ fertilization + precipitation, data=mydata)
fit.aov.int <- aov(log(NH3+100) ~ (fertilization + precipitation)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lm(log(NH3+100) ~ precipitation, data=mydata)
fit2 <- lm(log(NH3+100) ~ fertilization, data=mydata)
fit3 <- lm(log(NH3+100) ~ fertilization + precipitation, data=mydata)
...
# visualization
                # library(HH)
interaction2wt(log(NH3+100) ~ (fertilization + precipitation)^2, data=mydata, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# # outlier removed
# myfit <- lm(log(NH3+100) ~ fertilization + precipitation, data=mydata[-c(68)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
```
...
summary(powerTransform(mydata[,log(NH3+100)+1]))
```
...
### linearity
################################################################################
# boxTidwell(log(NH3+100) ~ fertilization + precipitation, data=mydata)
...
```{r}
library(leaps)
leaps <-regsubsets(log(NH3+100) ~ fertilization + precipitation, data=mydata, nbest=4)

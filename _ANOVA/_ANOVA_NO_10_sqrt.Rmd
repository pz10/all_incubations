---
title: "Nitric oxide (NO) differences. SQRT(NO) transformation"
...
```{r}
fit <- lm(sqrt(NO) ~ fertilization + precipitation + tillage, data=data)
fit.aov <- aov(sqrt(NO) ~ fertilization + precipitation + tillage, data=data)
fit.aov.int <- aov(sqrt(NO) ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(sqrt(NO) ~ precipitation + tillage, data=data)
fit2 <- lm(sqrt(NO) ~ fertilization + tillage, data=data)
fit3 <- lm(sqrt(NO) ~ fertilization + precipitation, data=data)
...
# outlier removed
myfit <- lm(sqrt(NO) ~ fertilization + precipitation + tillage, data=data[-c(17)])
gvmodel <- gvlma(myfit)
summary(gvmodel)
```
...
outlierTest(fit)
next.outlier <- lm(sqrt(NO) ~ fertilization + precipitation + tillage, data=data[-c(17)]) # only 17 is an outlier

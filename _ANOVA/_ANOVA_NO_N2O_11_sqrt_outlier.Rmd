...
```{r}
fit <- lm(sqrt(NO.N2O) ~ fertilization + precipitation + tillage, data=data[-c(68)])
fit.aov <- aov(sqrt(NO.N2O) ~ fertilization + precipitation + tillage, data=data[-c(68)])
fit.aov.int <- aov(sqrt(NO.N2O) ~ (fertilization + precipitation + tillage)^2,data=data[-c(68)])

# factor removal; for later model comparison
fit1 <- lm(sqrt(NO.N2O) ~ precipitation + tillage, data=data[-c(68)])
fit2 <- lm(sqrt(NO.N2O) ~ fertilization + tillage, data=data[-c(68)])
fit3 <- lm(sqrt(NO.N2O) ~ fertilization + precipitation, data=data[-c(68)])
...
# visualization
                # library(HH)
interaction2wt(sqrt(NO.N2O) ~ (fertilization + precipitation + tillage)^2, data=data[-c(68)], main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
```{r}
library(leaps)
leaps <-regsubsets(sqrt(NO.N2O) ~ fertilization + precipitation + tillage, data=data[-c(68)], nbest=4)

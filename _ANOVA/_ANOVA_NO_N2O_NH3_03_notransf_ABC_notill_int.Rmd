---
title: "NO+N2O+NH3 differences. Without transformation"
output: html_document
---
        
* Incubations A,B and C used (D has no NH3 data)
* Tillage not used in the OLR model
* model is defined with interactions

```{r}
data.ABC <- data[incubation != "D"]
# folowing summaries consider only incubations A:C
```
...
par(mfrow=c(1,4))
with(data.ABC, boxplot(NO.N2O.NH3 ~ fertilization, main = "fertilization"))
with(data.ABC, boxplot(NO.N2O.NH3 ~ precipitation, main = "precipitation"))
with(data.ABC, boxplot(NO.N2O.NH3 ~ tillage, main = "tillage"))
with(data.ABC, boxplot(NO.N2O.NH3 ~ incubation, main = "incubation"))

```
![NO.N2O.NH3 fluxes](auxiliary_plots_for_ANOVA_reports/boxplot_NO.N2O.NH3.png)
...
# now fit is defined with interaction
fit <- lm(NO.N2O.NH3 ~ (fertilization + precipitation)^2 , data=data.ABC)
fit.aov <- aov(NO.N2O.NH3 ~ (fertilization + precipitation)^2 , data=data.ABC)
fit.aov.without.int <- aov(NO.N2O.NH3 ~ fertilization + precipitation, data=data.ABC)

# factor removal; for later model comparison
fit1 <- lm(NO.N2O.NH3 ~ precipitation , data=data.ABC)
fit2 <- lm(NO.N2O.NH3 ~ fertilization , data=data.ABC)

# summary(fit)
summary(fit.aov)

# model with interactions
summary(fit.aov.without.int)

# visualization
# library(HH)
interaction2wt(NO.N2O.NH3 ~ (fertilization + precipitation )^2, data=data.ABC, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# NORMALITY
par(mfrow=c(1,1))
# qqPlot(fit, labels=data.ABC$id, simulate=TRUE, main="Q-Q Plot", id.method="identify")
# qqPlot(fit, labels=rownames(data.ABC), simulate=TRUE, main=paste("Q-Q Plot \n",fit$cal[2]), id.method="identify")
...
# crPlots(fit, main = paste(fit$call[2])) # not in models with interactions
```
...
# # outlier removed
# myfit <- lm(NO.N2O.NH3 ~ fertilization + precipitation , data=data.ABC[-c(68)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
```
...
cutoff <- 4/(nrow(data.ABC)-length(fit$coefficients)-2)
...
```{r}
avPlots(fit, ask=FALSE, id.method="y", id.n = 2, labels=rownames(data.ABC))
```
...
# influencePlot(fit, id.n = 2, labels=rownames(data.ABC),
...
summary(powerTransform(data.ABC[,NO.N2O.NH3]))
```
...
### linearity
################################################################################
# boxTidwell(NO.N2O.NH3 ~ fertilization + precipitation , data=data.ABC)
...
anova(fit, fit1)
anova(fit, fit2)
anova(fit, fit.aov.without.int)

```

<span style="color:red">
both factors add significance to the model
interaction term add to significance to the model
</span>
...
AIC(fit,fit1)
AIC(fit,fit2)
AIC(fit, fit.aov.without.int)
```

<span style="color:red">
2-factor model with interaction has the lowest AIC
</span>
...
```{r}
library(leaps)
leaps <-regsubsets(NO.N2O.NH3 ~ fertilization + precipitation , data=data.ABC, nbest=4)
...
<span style="color:red">
a 2 factorial model maximize adjusted R squared; if fertilization is not considered, adjR2 is reduced
</span>

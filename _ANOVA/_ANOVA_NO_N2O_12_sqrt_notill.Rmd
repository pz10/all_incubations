...
## tillage was removed from OLS model
...
```{r}
fit <- lm(sqrt(NO.N2O) ~ fertilization + precipitation , data=data)
fit.aov <- aov(sqrt(NO.N2O) ~ fertilization + precipitation , data=data)
fit.aov.int <- aov(sqrt(NO.N2O) ~ (fertilization + precipitation )^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(sqrt(NO.N2O) ~ precipitation , data=data)
fit2 <- lm(sqrt(NO.N2O) ~ fertilization , data=data)
...
# visualization
                # library(HH)
interaction2wt(sqrt(NO.N2O) ~ (fertilization + precipitation )^2, data=data, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
### linearity
################################################################################
# boxTidwell(sqrt(NO.N2O) ~ fertilization + precipitation , data=data)
...
<span style="color:red">
--> both facotrs add significant prediction power to the model
</span>
...
```{r}
library(leaps)
leaps <-regsubsets(sqrt(NO.N2O) ~ fertilization + precipitation , data=data, nbest=4)
...
<span style="color:red">
--> the model with both factors maximize adjusted R squared; if fertilization is not considered, adjR2 is reduced
</span>

---
title: "NH3 differences. log(NH3+1) transformation"
...
```{r}
fit <- lm(log(NH3+1) ~ incubation + fertilization + precipitation, data=mydata)
fit.aov <- aov(log(NH3+1) ~ incubation + fertilization + precipitation, data=mydata)
fit.aov.int <- aov(log(NH3+1) ~ (incubation + fertilization + precipitation)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lm(log(NH3+1) ~ incubation + precipitation, data=mydata)
fit2 <- lm(log(NH3+1) ~ incubation + fertilization, data=mydata)
fit3 <- lm(log(NH3+1) ~ fertilization + precipitation, data=mydata)
...
# visualization
                # library(HH)
interaction2wt(log(NH3+1) ~ (incubation + fertilization + precipitation)^2, data=mydata, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# # outlier removed
# myfit <- lm(log(NH3+1) ~ incubation + fertilization + precipitation, data=mydata[-c(68)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
```
...
summary(powerTransform(mydata[,log(NH3+1)+1]))
```
...
### linearity
################################################################################
# boxTidwell(log(NH3+1) ~ incubation + fertilization + precipitation, data=mydata)
...
<span style="color:red">
--> These 2-factors add prediction power to the model 
</span>
...
<span style="color:red">
--> 2-fator model has the lowest AIC
</span>
...
```{r}
library(leaps)
leaps <-regsubsets(log(NH3+1) ~ incubation + fertilization + precipitation, data=mydata, nbest=4)
...
<span style="color:red">
--> 2-fator model maximize adjusted R squared
</span>

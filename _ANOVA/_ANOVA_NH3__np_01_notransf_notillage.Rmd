...
```{r, results='hide'}
fit <- lmp(NH3 ~ fertilization + precipitation, data=mydata)
fit.aov <- aovp(NH3 ~ fertilization + precipitation, data=mydata)
fit.aov.int <- aovp(NH3 ~ (fertilization + precipitation)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lmp(NH3 ~ precipitation, data=mydata)
fit2 <- lmp(NH3 ~ fertilization, data=mydata)
...
# visualization
                # library(HH)
interaction2wt(NH3 ~ (fertilization + precipitation)^2, data=mydata, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# 
# # # outlier removed
# # myfit <- lmp(NH3 ~ fertilization + precipitation, data=mydata[-c(68)])
# # gvmodel <- gvlma(myfit)
# # summary(gvmodel)
```
...
### linearity
################################################################################
# boxTidwell(NH3 ~ fertilization + precipitation, data=mydata)
...
```{r}
library(leaps)
leaps <-regsubsets(NH3 ~ fertilization + precipitation, data=mydata, nbest=4)

---
title: "GWP.CO2eq differences. Approaches: Non-parametric and parametric (sqrt transformation) Interaction considered"
output: html_document
---

GWP.CO2eq = GWP of CO2+N2O+CH4 in mg CO2 m-2 h-1

```{r, echo=F}
data <- copy(RAWdata)

data[,.(
        mean.GWP.CO2eq = mean(GWP.CO2eq, na.rm=T),
        sd.GWP.CO2eq = sd(GWP.CO2eq, na.rm=T),
        se.GWP.CO2eq = sd(GWP.CO2eq, na.rm=T)/sqrt(sum(!is.na(GWP.CO2eq))),
        count.GWP.CO2eq = sum(!is.na(GWP.CO2eq))
), by=fertilization]
...
par(mfrow=c(1,4))
with(data, boxplot(sqrt(GWP.CO2eq) ~ fertilization, main = "fertilization"))
with(data, boxplot(sqrt(GWP.CO2eq) ~ precipitation, main = "precipitation"))
with(data, boxplot(sqrt(GWP.CO2eq) ~ tillage, main = "tillage"))
with(data, boxplot(sqrt(GWP.CO2eq) ~ incubation, main = "incubation"))


par(mfrow=c(1,4))
ggplot(data=data, aes(x=fertilization, y=GWP.CO2eq)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=data, aes(x=precipitation, y=GWP.CO2eq)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=data, aes(x=tillage, y=GWP.CO2eq)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=data, aes(x=incubation, y=GWP.CO2eq)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
...
```{r, echo=F}
# replace strings:
# 1. "sqrt(GWP.CO2eq) ~" --> "transformation ~"
# 2. "[,sqrt(GWP.CO2eq)]" --> "[,transformation]"
```

```{r}
fit <- lm(sqrt(GWP.CO2eq) ~ fertilization + precipitation + tillage, data=data)
fit.aov <- aov(sqrt(GWP.CO2eq) ~ fertilization + precipitation + tillage, data=data)
fit.aov.int <- aov(sqrt(GWP.CO2eq) ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(sqrt(GWP.CO2eq) ~  precipitation + tillage, data=data)
fit2 <- lm(sqrt(GWP.CO2eq) ~  fertilization + tillage, data=data)
fit3 <- lm(sqrt(GWP.CO2eq) ~  fertilization + precipitation, data=data)

fit.int <- lm(sqrt(GWP.CO2eq) ~ ( fertilization + precipitation + tillage)^2, data=data)

fit <- lm(sqrt(GWP.CO2eq) ~ fertilization + precipitation + tillage + fertilization:precipitation + fertilization:tillage, data=data)
fit.aov <- fit.aov <- aov(sqrt(GWP.CO2eq) ~ fertilization + precipitation + tillage + fertilization:precipitation + fertilization:tillage, data=data)
...
```{r}
fit.p <- lmp(sqrt(GWP.CO2eq) ~  fertilization + precipitation + tillage, data=data)
fit.aovp <- aovp(sqrt(GWP.CO2eq) ~  fertilization + precipitation + tillage, data=data)
fit.aovp.int <- aovp(sqrt(GWP.CO2eq) ~ ( fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1.p <- lmp(sqrt(GWP.CO2eq) ~  precipitation + tillage, data=data)
fit2.p <- lmp(sqrt(GWP.CO2eq) ~  fertilization + tillage, data=data)
fit3.p <- lmp(sqrt(GWP.CO2eq) ~  fertilization + precipitation, data=data)

fit.int.p <- lmp(sqrt(GWP.CO2eq) ~ ( fertilization + precipitation + tillage)^2, data=data)
...
# 
# # outlier removed
# myfit <- lm(sqrt(GWP.CO2eq) ~ fertilization + precipitation + tillage, data=data[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(sqrt(GWP.CO2eq) ~ fertilization + precipitation + tillage, data=data[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(sqrt(GWP.CO2eq) ~ fertilization + precipitation + tillage, data=data[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(sqrt(GWP.CO2eq) ~ fertilization + precipitation + tillage, data=data[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(sqrt(GWP.CO2eq) ~ fertilization + precipitation + tillage, data=data[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
# myoutliers <- c(68,54)
# data[myoutliers, .(id, GWP.CO2eq, CO2.1.3, GWP.CO2eq, CO2.1.3)]
# 
...
summary(powerTransform(data[,sqrt(GWP.CO2eq)]+1))
```
...
### linearity
################################################################################
# boxTidwell(sqrt(GWP.CO2eq) ~ fertilization + precipitation + tillage, data=data)
...
```{r}
library(leaps)
leaps <-regsubsets(sqrt(GWP.CO2eq) ~ ( fertilization + precipitation + tillage), data=data, nbest=4)
...
```{r, echo=F}
# fert
results.fert <- W.MW.oneway(sqrt(GWP.CO2eq) ~ fertilization, data, n=my.n)
summary(results.fert)
plot(results.fert)

# precipitation
results.rain <- W.MW.oneway(sqrt(GWP.CO2eq) ~ precipitation, data, n=my.n)
summary(results.rain)
plot(results.rain)

# tillage
results.tillage <- W.MW.oneway(sqrt(GWP.CO2eq) ~ tillage, data, n=my.n)
...
```{r, echo=F}
# # fert
# results.fert <- PToneway(sqrt(GWP.CO2eq) ~ fertilization, data, n=my.n)
# summary.oneway(results.fert)
# 
# # precipitation
# results.rain <- PToneway(sqrt(GWP.CO2eq) ~ precipitation, data, n=my.n)
# summary.oneway(results.rain)
# 
# # tillage
# results.till <- PToneway(sqrt(GWP.CO2eq) ~ tillage, data, n=my.n)

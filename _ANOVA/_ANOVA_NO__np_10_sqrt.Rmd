---
title: "Nitric oxide (NO) differences (permutation test). SQRT(NO) transformation"
...
par(mfrow=c(1,4))
with(data, boxplot(NO ~ fertilization, main = "fertilization"))
with(data, boxplot(NO ~ precipitation, main = "precipitation"))
with(data, boxplot(NO ~ tillage, main = "tillage"))
with(data, boxplot(NO ~ incubation, main = "incubation"))
...
```
![No fluxes](auxiliary_plots_for_ANOVA_reports/boxplot_NO.png)

# 0 conclusions

<span style="color:red">
--> sqrt(NO) transformation model fulfill assumptions;  conclusions do not differ much from non-transformed model
</span>
...
```{r, echo=F}
# replace strings:
# 1. "sqrt(NO) ~" --> "transformation ~"
# 2. "[,sqrt(NO)]" --> "[,transformation]"
```

```{r, results="hide"}
fit <- lmp(sqrt(NO) ~ fertilization + precipitation + tillage, data=data)
fit.aov <- aovp(sqrt(NO) ~ fertilization + precipitation + tillage, data=data)
fit.aov.int <- aovp(sqrt(NO) ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lmp(sqrt(NO) ~ precipitation + tillage, data=data)
fit2 <- lmp(sqrt(NO) ~ fertilization + tillage, data=data)
fit3 <- lmp(sqrt(NO) ~ fertilization + precipitation, data=data)
```
...
# visualization
                # library(HH)
interaction2wt(sqrt(NO) ~ (fertilization + precipitation + tillage)^2, data=data, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# 
# # outlier removed
# myfit <- lmp(sqrt(NO) ~ fertilization + precipitation + tillage, data=data[-c(17)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
```
...
outlierTest(fit)
next.outlier <- lmp(sqrt(NO) ~ fertilization + precipitation + tillage, data=data[-c(17)]) # only 17 is an outlier
...
summary(powerTransform(data[,sqrt(NO)]))
```

<span style="color:red">
--> null hypothesis λ = 1 cannont be rejected: no transformation suggested
</span>
...
### linearity
################################################################################
# boxTidwell(sqrt(NO) ~ fertilization + precipitation + tillage, data=data)
...
<span style="color:red">
--> All factors add prediction value to the model
</span>
...
```{r}
library(leaps)
leaps <-regsubsets(sqrt(NO) ~ fertilization + precipitation + tillage, data=data, nbest=4)
...
<span style="color:red">
--> a full factorial model maximize adjusted R squared
</span>
...
<span style="color:red">
--> precipitaion i-h difference is now significant with applied transformation
</span>
---
title: "NH3 differences (Permutation test). Without transformation"
output: html_document
---

```{r echo=FALSE}                                 
mydata <- data[!is.na(NH3),]
``` 
...
```
![NH3 fluxes](auxiliary_plots_for_ANOVA_reports/boxplot_NH3.png)

# 0 conclusions

<div style="color:red">

* log transformation suggested. However, assumprions under log model are not satisfied neither.
* non-transformed model have non-constant error variance (too many zero values; low NH3 emissions have less variance than higher NH3 emissions)

* conclude that fertilization enhances NH3 volatization
* conclude that decreasing rain pattern attenuuates NH3 volatization compared to increasing pattern
</div>
...
```{r, echo=F}
# replace strings:
# 1. "NH3 ~" --> "transformation ~"
# 2. "[,NH3]" --> "[,transformation]"
```

```{r,results='hide'}
fit <- lmp(NH3 ~ fertilization + precipitation + tillage, data=mydata)
fit.aov <- aovp(NH3 ~ fertilization + precipitation + tillage, data=mydata)
fit.aov.int <- aovp(NH3 ~ (fertilization + precipitation + tillage)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lmp(NH3 ~ precipitation + tillage, data=mydata)
fit2 <- lmp(NH3 ~ fertilization + tillage, data=mydata)
fit3 <- lmp(NH3 ~ fertilization + precipitation, data=mydata)
...
# visualization
                # library(HH)
interaction2wt(NH3 ~ (fertilization + precipitation + tillage)^2, data=mydata, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# 
# # # outlier removed
# # myfit <- lmp(NH3 ~ fertilization + precipitation + tillage, data=mydata[-c(68)])
# # gvmodel <- gvlma(myfit)
# # summary(gvmodel)
```
...
# # identify outliers
# myoutliers <- c(68,54)
# mydata[myoutliers, .(id, NO, N2O, NH3, NO.N2O, NO.N2O.NH3)]
# 
...
summary(powerTransform(mydata[,NH3+1]))
```

<span style="color:red">
--> null hypothesis λ = 1 can be rejected; possible transformation: log
</span>
...
### linearity
################################################################################
# boxTidwell(NH3 ~ fertilization + precipitation + tillage, data=mydata)
...
<span style="color:red">
--> removing tillage from full (3) factorial model reduces AIC
</span>
...
```{r}
library(leaps)
leaps <-regsubsets(NH3 ~ fertilization + precipitation + tillage, data=mydata, nbest=4)
...
<span style="color:red">
--> removing tillage from full (3) factorial model maximize adjusted R squared
</span>

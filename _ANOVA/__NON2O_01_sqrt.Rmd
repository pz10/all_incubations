---
title: "NO+N2O differences. Approaches: Non-parametric and parametric (sqrt transformation)"
...
par(mfrow=c(1,4))
with(data, boxplot(sqrt(NO.N2O) ~ fertilization, main = "fertilization"))
with(data, boxplot(sqrt(NO.N2O) ~ precipitation, main = "precipitation"))
with(data, boxplot(sqrt(NO.N2O) ~ tillage, main = "tillage"))
with(data, boxplot(sqrt(NO.N2O) ~ incubation, main = "incubation"))
...
fit.int <- lm(sqrt(NO.N2O) ~ ( fertilization + precipitation + tillage)^2, data=data)
...
```{r}
fit.p <- lmp(sqrt(NO.N2O) ~  fertilization + precipitation + tillage, data=data)
fit.aovp <- aovp(sqrt(NO.N2O) ~  fertilization + precipitation + tillage, data=data)
fit.aovp.int <- aovp(sqrt(NO.N2O) ~ ( fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1.p <- lmp(sqrt(NO.N2O) ~  precipitation + tillage, data=data)
fit2.p <- lmp(sqrt(NO.N2O) ~  fertilization + tillage, data=data)
fit3.p <- lmp(sqrt(NO.N2O) ~  fertilization + precipitation, data=data)

fit.int.p <- lmp(sqrt(NO.N2O) ~ ( fertilization + precipitation + tillage)^2, data=data)
...
# 
# # outlier removed
# myfit <- lm(sqrt(NO.N2O) ~ fertilization + precipitation + tillage, data=data[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(sqrt(NO.N2O) ~ fertilization + precipitation + tillage, data=data[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(sqrt(NO.N2O) ~ fertilization + precipitation + tillage, data=data[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(sqrt(NO.N2O) ~ fertilization + precipitation + tillage, data=data[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(sqrt(NO.N2O) ~ fertilization + precipitation + tillage, data=data[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
...
summary(powerTransform(data[,sqrt(NO.N2O)]+1))
```
...
```{r, echo=F}
# fert
results.fert <- W.MW.oneway(sqrt(NO.N2O) ~ fertilization, data, n=my.n)
summary(results.fert)
plot(results.fert)

# precipitation
results.rain <- W.MW.oneway(sqrt(NO.N2O) ~ precipitation, data, n=my.n)
summary(results.rain)
plot(results.rain)

# tillage
results.tillage <- W.MW.oneway(sqrt(NO.N2O) ~ tillage, data, n=my.n)
...
```{r, echo=F}
# fert
# results.fert <- PToneway(sqrt(NO.N2O) ~ fertilization, data, n=my.n)
# summary.oneway(results.fert)
# 
# # precipitation
# results.rain <- PToneway(sqrt(NO.N2O) ~ precipitation, data, n=my.n)
# summary.oneway(results.rain)
# 
# # tillage
# results.till <- PToneway(sqrt(NO.N2O) ~ tillage, data, n=my.n)

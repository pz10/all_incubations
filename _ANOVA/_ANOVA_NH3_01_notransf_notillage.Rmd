...
```{r}
fit <- lm(NH3 ~ fertilization + precipitation, data=mydata)
fit.aov <- aov(NH3 ~ fertilization + precipitation, data=mydata)
fit.aov.int <- aov(NH3 ~ (fertilization + precipitation)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lm(NH3 ~ precipitation, data=mydata)
fit2 <- lm(NH3 ~ fertilization, data=mydata)
...
# # outlier removed
# myfit <- lm(NH3 ~ fertilization + precipitation, data=mydata[-c(68)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
```

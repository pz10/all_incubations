---
title: "Nitric oxide (NO) differences. Without transformation"
...
```{r}
fit <- lm(NO ~ fertilization + precipitation + tillage, data=data)
fit.aov <- aov(NO ~ fertilization + precipitation + tillage, data=data)
fit.aov.int <- aov(NO ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(NO ~ precipitation + tillage, data=data)
fit2 <- lm(NO ~ fertilization + tillage, data=data)
fit3 <- lm(NO ~ fertilization + precipitation, data=data)
...
# outlier removed
myfit <- lm(NO ~ fertilization + precipitation + tillage, data=data[-c(17)])
gvmodel <- gvlma(myfit)
summary(gvmodel)
```
...
outlierTest(fit)
next.outlier <- lm(NO ~ fertilization + precipitation + tillage, data=data[-c(17)]) # only 17 is an outlier

---
title: "NH3 (incubations A,B,C) differences. Approaches: Non-parametric and parametric (log transformation)"
...
```{r}
fit <- lm(log(NH3+1) ~ incubation + fertilization + precipitation + tillage, data=data)
fit.aov <- aov(log(NH3+1) ~ incubation + fertilization + precipitation + tillage, data=data)
fit.aov.int <- aov(log(NH3+1) ~ (incubation + fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(log(NH3+1) ~ incubation + precipitation + tillage, data=data)
fit2 <- lm(log(NH3+1) ~ incubation + fertilization + tillage, data=data)
fit3 <- lm(log(NH3+1) ~ incubation + fertilization + precipitation, data=data)

fit.int <- lm(log(NH3+1) ~ (incubation + fertilization + precipitation + tillage)^2, data=data)
...
```{r}
library(leaps)
leaps <-regsubsets(log(NH3+1) ~ (incubation + fertilization + precipitation + tillage), data=data, nbest=4)

---
title: "Nitric oxide (NO) differences (permutation test). Without transformation"
...
<div style="color:red">
--> square root transformation suggested to achieve a constant residual variance
</div>
...
```{r, results="hide"}
fit <- lmp(NO ~ fertilization + precipitation + tillage, data=data)
fit.aov <- aovp(NO ~ fertilization + precipitation + tillage, data=data)
fit.aov.int <- aovp(NO ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lmp(NO ~ precipitation + tillage, data=data)
fit2 <- lmp(NO ~ fertilization + tillage, data=data)
fit3 <- lmp(NO ~ fertilization + precipitation, data=data)
```
...
# visualization
                # library(HH)
interaction2wt(NO ~ (fertilization + precipitation + tillage)^2, data=data, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# 
# # outlier removed
# myfit <- lmp(NO ~ fertilization + precipitation + tillage, data=data[-c(17)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
```
...
outlierTest(fit)
next.outlier <- lmp(NO ~ fertilization + precipitation + tillage, data=data[-c(17)]) # only 17 is an outlier
outlierTest(next.outlier)

# identify outliers
data[c(17), .(id, NO, N2O, NH3, NO.N2O, NO.N2O.NH3)]

data[c(17), NO]; fitted(fit)[c(17)]
residuals(fit)[c(17)]; rstudent(fit)[c(17)]
```
...
summary(powerTransform(data[,NO]))
```

<div style="color:red">
--> null hypothesis λ = 1 can be rejected; possible transformation: square root
</div>
...
### linearity
################################################################################
# boxTidwell(NO ~ fertilization + precipitation + tillage, data=data)
...
```{r}
library(leaps)
leaps <-regsubsets(NO ~ fertilization + precipitation + tillage, data=data, nbest=4)

...
---
## Tillage not considered in the OLS model

## Incubation considered in the OLS model
```{r echo=FALSE}                                 
...
Including incubation in the model might be a good idea. It might be better with  a log transformation
</span>
...
```{r, results='hide'}
fit <- lmp(NH3 ~ incubation + fertilization + precipitation, data=mydata)
fit.aov <- aovp(NH3 ~ incubation + fertilization + precipitation, data=mydata)
fit.aov.int <- aovp(NH3 ~ (incubation + fertilization + precipitation)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lmp(NH3 ~ incubation + precipitation, data=mydata)
fit2 <- lmp(NH3 ~ incubation + fertilization, data=mydata)
...
# visualization
                # library(HH)
interaction2wt(NH3 ~ (incubation + fertilization + precipitation)^2, data=mydata, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# 
# # # outlier removed
# # myfit <- lmp(NH3 ~ incubation + fertilization + precipitation, data=mydata[-c(68)])
# # gvmodel <- gvlma(myfit)
# # summary(gvmodel)
```
...
### linearity
################################################################################
# boxTidwell(NH3 ~ incubation + fertilization + precipitation, data=mydata)
...
<span style="color:red">
--> All 3-factors add prediction power to the model 
</span>
...
<span style="color:red">
--> A 3-factor model has the lowest AIC
</span>
...
<span style="color:red">
--> 3-fator model has the lowest AIC
</span>

#### ALL SUBSETS REGRESSION
```{r}
library(leaps)
leaps <-regsubsets(NH3 ~ incubation + fertilization + precipitation, data=mydata, nbest=4)
...
<span style="color:red">
--> 3-fator model maximize adjusted R squared
</span>

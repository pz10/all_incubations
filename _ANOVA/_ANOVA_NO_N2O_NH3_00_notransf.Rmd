...
---
All incubations were used (A,b,C,D). D-NH3 as mean.NH3(A,B,C)
...
```{r}
fit <- lm(NO.N2O.NH3 ~ fertilization + precipitation + tillage, data=data)
fit.aov <- aov(NO.N2O.NH3 ~ fertilization + precipitation + tillage, data=data)
fit.aov.int <- aov(NO.N2O.NH3 ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(NO.N2O.NH3 ~ precipitation + tillage, data=data)
fit2 <- lm(NO.N2O.NH3 ~ fertilization + tillage, data=data)
fit3 <- lm(NO.N2O.NH3 ~ fertilization + precipitation, data=data)
...
# visualization
                # library(HH)
interaction2wt(NO.N2O.NH3 ~ (fertilization + precipitation + tillage)^2, data=data, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# # outlier removed
# myfit <- lm(NO.N2O.NH3 ~ fertilization + precipitation + tillage, data=data[-c(68)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
```
...
```{r}
library(leaps)
leaps <-regsubsets(NO.N2O.NH3 ~ fertilization + precipitation + tillage, data=data, nbest=4)

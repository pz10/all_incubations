---
title: "NO+N2O+NH3 differences. sqrt(NO + N2O + NH3) transformation"
...
```{r}
fit <- lm(sqrt(NO.N2O.NH3) ~ fertilization + precipitation + tillage, data=data.ABC)
fit.aov <- aov(sqrt(NO.N2O.NH3) ~ fertilization + precipitation + tillage, data=data.ABC)
fit.aov.int <- aov(sqrt(NO.N2O.NH3) ~ (fertilization + precipitation + tillage)^2, data=data.ABC)

# factor removal; for later model comparison
fit1 <- lm(sqrt(NO.N2O.NH3) ~ precipitation + tillage, data=data.ABC)
fit2 <- lm(sqrt(NO.N2O.NH3) ~ fertilization + tillage, data=data.ABC)
fit3 <- lm(sqrt(NO.N2O.NH3) ~ fertilization + precipitation, data=data.ABC)
...
# visualization
                # library(HH)
interaction2wt(sqrt(NO.N2O.NH3) ~ (fertilization + precipitation + tillage)^2, data=data.ABC, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# # outlier removed
# myfit <- lm(sqrt(NO.N2O.NH3) ~ fertilization + precipitation + tillage, data=data.ABC[-c(68)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
...
### linearity
################################################################################
# boxTidwell(sqrt(NO.N2O.NH3) ~ fertilization + precipitation + tillage, data=data.ABC)
...
<span style="color:red">
--> factorial model without tillage has the lowest AIC
</span>
...
```{r}
library(leaps)
leaps <-regsubsets(sqrt(NO.N2O.NH3) ~ fertilization + precipitation + tillage, data=data.ABC, nbest=4)

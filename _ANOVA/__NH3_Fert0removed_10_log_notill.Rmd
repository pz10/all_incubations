---
title: "NH3 (incubations A,B,C) differences. Fert=0 removed. Approaches: Non-parametric and parametric (log transformation; tillage removed from model)"
...
```{r}
fit <- lm(log(NH3) ~ incubation + fertilization + precipitation , data=data)
fit.aov <- aov(log(NH3) ~ incubation + fertilization + precipitation , data=data)
fit.aov.int <- aov(log(NH3) ~ (incubation + fertilization + precipitation )^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(log(NH3) ~ incubation + precipitation , data=data)
fit2 <- lm(log(NH3) ~ incubation + fertilization , data=data)
fit3 <- lm(log(NH3) ~ incubation + fertilization + precipitation, data=data)

fit.int <- lm(log(NH3) ~ (incubation + fertilization + precipitation )^2, data=data)
...
```{r}
fit.p <- lmp(NH3 ~ incubation + fertilization + precipitation , data=data)
fit.aovp <- aovp(NH3 ~ incubation + fertilization + precipitation , data=data)
fit.aovp.int <- aovp(NH3 ~ (incubation + fertilization + precipitation )^2, data=data)

# factor removal; for later model comparison
fit1.p <- lmp(NH3 ~ incubation + precipitation , data=data)
fit2.p <- lmp(NH3 ~ incubation + fertilization , data=data)
fit3.p <- lmp(NH3 ~ incubation + fertilization + precipitation, data=data)

fit.int.p <- lmp(NH3 ~ (incubation + fertilization + precipitation )^2, data=data)
...
# 
# # outlier removed
# myfit <- lm(NH3 ~ fertilization + precipitation , data=data[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(NH3 ~ fertilization + precipitation , data=data[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(NH3 ~ fertilization + precipitation , data=data[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(NH3 ~ fertilization + precipitation , data=data[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(NH3 ~ fertilization + precipitation , data=data[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
...
### linearity
################################################################################
# boxTidwell(NH3 ~ fertilization + precipitation , data=data)
...
```{r}
library(leaps)
leaps <-regsubsets(log(NH3) ~ (incubation + fertilization + precipitation ), data=data, nbest=4)

---
title: "GWP.CO2eq differences. Approaches: Non-parametric and parametric (no transformation) Interaction considered"
...
par(mfrow=c(1,4))
with(data, boxplot(GWP.CO2eq ~ fertilization, main = "fertilization"))
with(data, boxplot(GWP.CO2eq ~ precipitation, main = "precipitation"))
with(data, boxplot(GWP.CO2eq ~ tillage, main = "tillage"))
with(data, boxplot(GWP.CO2eq ~ incubation, main = "incubation"))
...
```{r, echo=F}
# replace strings:
# 1. "GWP.CO2eq ~" --> "transformation ~"
# 2. "[,GWP.CO2eq]" --> "[,transformation]"
```

```{r}
fit <- lm(GWP.CO2eq ~ fertilization + precipitation + tillage, data=data)
fit.aov <- aov(GWP.CO2eq ~ fertilization + precipitation + tillage, data=data)
fit.aov.int <- aov(GWP.CO2eq ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(GWP.CO2eq ~  precipitation + tillage, data=data)
fit2 <- lm(GWP.CO2eq ~  fertilization + tillage, data=data)
fit3 <- lm(GWP.CO2eq ~  fertilization + precipitation, data=data)

fit.int <- lm(GWP.CO2eq ~ ( fertilization + precipitation + tillage)^2, data=data)

fit <- lm(GWP.CO2eq ~ fertilization + precipitation + tillage + fertilization:precipitation + fertilization:tillage, data=data)
fit.aov <- fit.aov <- aov(GWP.CO2eq ~ fertilization + precipitation + tillage + fertilization:precipitation + fertilization:tillage, data=data)
...
```{r}
fit.p <- lmp(GWP.CO2eq ~  fertilization + precipitation + tillage, data=data)
fit.aovp <- aovp(GWP.CO2eq ~  fertilization + precipitation + tillage, data=data)
fit.aovp.int <- aovp(GWP.CO2eq ~ ( fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1.p <- lmp(GWP.CO2eq ~  precipitation + tillage, data=data)
fit2.p <- lmp(GWP.CO2eq ~  fertilization + tillage, data=data)
fit3.p <- lmp(GWP.CO2eq ~  fertilization + precipitation, data=data)

fit.int.p <- lmp(GWP.CO2eq ~ ( fertilization + precipitation + tillage)^2, data=data)
...
# 
# # outlier removed
# myfit <- lm(GWP.CO2eq ~ fertilization + precipitation + tillage, data=data[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(GWP.CO2eq ~ fertilization + precipitation + tillage, data=data[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(GWP.CO2eq ~ fertilization + precipitation + tillage, data=data[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(GWP.CO2eq ~ fertilization + precipitation + tillage, data=data[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(GWP.CO2eq ~ fertilization + precipitation + tillage, data=data[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
...
summary(powerTransform(data[,GWP.CO2eq]+1))
```
...
### linearity
################################################################################
# boxTidwell(GWP.CO2eq ~ fertilization + precipitation + tillage, data=data)
...
```{r}
library(leaps)
leaps <-regsubsets(GWP.CO2eq ~ ( fertilization + precipitation + tillage), data=data, nbest=4)
...
```{r, echo=F}
# fert
results.fert <- W.MW.oneway(GWP.CO2eq ~ fertilization, data, n=my.n)
summary(results.fert)
plot(results.fert)

# precipitation
results.rain <- W.MW.oneway(GWP.CO2eq ~ precipitation, data, n=my.n)
summary(results.rain)
plot(results.rain)

# tillage
results.tillage <- W.MW.oneway(GWP.CO2eq ~ tillage, data, n=my.n)
...
```{r, echo=F}
# fert
results.fert <- PToneway(GWP.CO2eq ~ fertilization, data, n=my.n)
summary.oneway(results.fert)

# precipitation
results.rain <- PToneway(GWP.CO2eq ~ precipitation, data, n=my.n)
summary.oneway(results.rain)

# tillage
results.till <- PToneway(GWP.CO2eq ~ tillage, data, n=my.n)

---
title: "3rd stretch NO+N2O+NH3 differences. Approaches: Non-parametric and parametric (no transformation)"
...
```{r}
fit <- lm(log(NO.N2O.NH3) ~ fertilization + precipitation + tillage, data=mydata)
fit.aov <- aov(log(NO.N2O.NH3) ~ fertilization + precipitation + tillage, data=mydata)
fit.aov.int <- aov(log(NO.N2O.NH3) ~ (fertilization + precipitation + tillage)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lm(log(NO.N2O.NH3) ~  precipitation + tillage, data=mydata)
fit2 <- lm(log(NO.N2O.NH3) ~  fertilization + tillage, data=mydata)
fit3 <- lm(log(NO.N2O.NH3) ~  fertilization + precipitation, data=mydata)

fit.int <- lm(log(NO.N2O.NH3) ~ ( incubation + fertilization + precipitation + tillage )^2, data=mydata)

# fit <- lm(log(NO.N2O.NH3) ~ incubation + fertilization + precipitation +  tillage + incubation:precipitation + fertilization:tillage, data=mydata)
# fit.aov <- fit.aov <- aov(log(NO.N2O.NH3) ~ fertilization + precipitation + tillage + fertilization:precipitation + fertilization:tillage, data=mydata)
...
```{r}
fit.p <- lmp(log(NO.N2O.NH3) ~  fertilization + precipitation + tillage, data=mydata)
fit.aovp <- aovp(log(NO.N2O.NH3) ~  fertilization + precipitation + tillage, data=mydata)
fit.aovp.int <- aovp(log(NO.N2O.NH3) ~ ( fertilization + precipitation + tillage)^2, data=mydata)

# factor removal; for later model comparison
fit1.p <- lmp(log(NO.N2O.NH3) ~  precipitation + tillage, data=mydata)
fit2.p <- lmp(log(NO.N2O.NH3) ~  fertilization + tillage, data=mydata)
fit3.p <- lmp(log(NO.N2O.NH3) ~  fertilization + precipitation, data=mydata)

fit.int.p <- lmp(log(NO.N2O.NH3) ~ ( fertilization + precipitation + tillage)^2, data=mydata)
...
# 
# # outlier removed
# myfit <- lm(log(NO.N2O.NH3) ~ fertilization + precipitation + tillage, data=mydata[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(log(NO.N2O.NH3) ~ fertilization + precipitation + tillage, data=mydata[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(log(NO.N2O.NH3) ~ fertilization + precipitation + tillage, data=mydata[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(log(NO.N2O.NH3) ~ fertilization + precipitation + tillage, data=mydata[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(log(NO.N2O.NH3) ~ fertilization + precipitation + tillage, data=mydata[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
...
summary(powerTransform(mydata[,log(NO.N2O.NH3)+2]))
```
...
### linearity
################################################################################
# boxTidwell(log(NO.N2O.NH3) ~ fertilization + precipitation + tillage, data=mydata)
...
```{r}
library(leaps)
leaps <-regsubsets(log(NO.N2O.NH3) ~ ( fertilization + precipitation + tillage), data=mydata, nbest=4)
...
```{r, echo=F}
# fert
results.fert <- W.MW.oneway(log(NO.N2O.NH3) ~ fertilization, mydata, n=my.n)
summary(results.fert)
# plot(results.fert)

# precipitation
results.rain <- W.MW.oneway(log(NO.N2O.NH3) ~ precipitation, mydata, n=my.n)
...
```{r, echo=F}
# # fert
# results.fert <- PToneway(log(NO.N2O.NH3) ~ fertilization, data = mydata, n=my.n)
# summary.oneway(results.fert)
# 
# # precipitation
# results.rain <- PToneway(log(NO.N2O.NH3) ~ precipitation, data = mydata, n=my.n)
# summary.oneway(results.rain)
# 
# # tillage
# results.till <- PToneway(log(NO.N2O.NH3) ~ tillage, data = mydata, n=my.n)

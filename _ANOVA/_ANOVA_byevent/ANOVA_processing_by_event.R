...
require(coin)
require(lmPerm)
source("_ANOVA/_ANOVA_byevent/W.MW.oneway.R")

# define output directory
my.output.dir <- "G:/BioAtmo/zuazo-p/General/eclaire/Automatic system/OUTPUTS/drying_wetting/_all_fluxes/_ANOVA/CumFlux_byevent"
dir.create(my.output.dir)

# load data
folder.in <-paste0(output_path, "/cum_fluxes_14/cumFlux_by_event")
myfile <- paste0(folder.in, "/EventcumFlux_bycore.dat")
data <- fread(myfile)
...
data[,id:= paste0(incubation, fertilization, precipitation, tillage, paste0("-",event))]


my.files <- list.files("_ANOVA/_ANOVA_byevent", pattern = ".Rmd", full.names = TRUE)
my.files
render(input = my.files[15], output_format = "html_document", output_dir = my.output.dir)

...
```{r}
fit <- lm(log(CO2) ~ precipitation + tillage, data=data)
fit.aov <- aov(log(CO2) ~ precipitation + tillage, data=data)
fit.aov.int <- aov(log(CO2) ~ (precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(log(CO2) ~ precipitation + tillage, data=data)
fit2 <- lm(log(CO2) ~ tillage, data=data)
fit3 <- lm(log(CO2) ~ precipitation, data=data)
...
# visualization
                # library(HH)
interaction2wt(log(CO2) ~ (precipitation + tillage)^2, data=data, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```

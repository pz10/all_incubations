---
title: "1st stretch NO+N2O differences. Approaches: Non-parametric and parametric (log transformation)"
...
par(mfrow=c(1,4))
with(mydata, boxplot(log(NO.N2O) ~ fertilization, main = "fertilization"))
with(mydata, boxplot(log(NO.N2O) ~ precipitation, main = "precipitation"))
with(mydata, boxplot(log(NO.N2O) ~ tillage, main = "tillage"))
with(mydata, boxplot(log(NO.N2O) ~ incubation, main = "incubation"))


par(mfrow=c(1,4))
ggplot(data=mydata, aes(x=fertilization, y=NO.N2O)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=mydata, aes(x=precipitation, y=NO.N2O)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=mydata, aes(x=tillage, y=NO.N2O)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=mydata, aes(x=incubation, y=NO.N2O)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
...
```{r, echo=F}
# replace strings:
# 1. "log(NO.N2O) ~" --> "transformation ~"
# 2. "[,log(NO.N2O)]" --> "[,transformation]"
```

```{r}
fit <- lm(log(NO.N2O) ~ fertilization + precipitation + tillage, data=mydata)
fit.aov <- aov(log(NO.N2O) ~ fertilization + precipitation + tillage, data=mydata)
fit.aov.int <- aov(log(NO.N2O) ~ (fertilization + precipitation + tillage)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lm(log(NO.N2O) ~  precipitation + tillage, data=mydata)
fit2 <- lm(log(NO.N2O) ~  fertilization + tillage, data=mydata)
fit3 <- lm(log(NO.N2O) ~  fertilization + precipitation, data=mydata)

fit.int <- lm(log(NO.N2O) ~ ( fertilization + precipitation + tillage)^2, data=mydata)

# fit <- lm(log(NO.N2O) ~ fertilization + precipitation + tillage + fertilization:precipitation + fertilization:tillage, data=mydata)
# fit.aov <- fit.aov <- aov(log(NO.N2O) ~ fertilization + precipitation + tillage + fertilization:precipitation + fertilization:tillage, data=mydata)
...
```{r}
fit.p <- lmp(log(NO.N2O) ~  fertilization + precipitation + tillage, data=mydata)
fit.aovp <- aovp(log(NO.N2O) ~  fertilization + precipitation + tillage, data=mydata)
fit.aovp.int <- aovp(log(NO.N2O) ~ ( fertilization + precipitation + tillage)^2, data=mydata)

# factor removal; for later model comparison
fit1.p <- lmp(log(NO.N2O) ~  precipitation + tillage, data=mydata)
fit2.p <- lmp(log(NO.N2O) ~  fertilization + tillage, data=mydata)
fit3.p <- lmp(log(NO.N2O) ~  fertilization + precipitation, data=mydata)

fit.int.p <- lmp(log(NO.N2O) ~ ( fertilization + precipitation + tillage)^2, data=mydata)
...
# 
# # outlier removed
# myfit <- lm(log(NO.N2O) ~ fertilization + precipitation + tillage, data=mydata[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(log(NO.N2O) ~ fertilization + precipitation + tillage, data=mydata[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(log(NO.N2O) ~ fertilization + precipitation + tillage, data=mydata[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(log(NO.N2O) ~ fertilization + precipitation + tillage, data=mydata[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(log(NO.N2O) ~ fertilization + precipitation + tillage, data=mydata[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
# myoutliers <- c(68,54)
# mydata[myoutliers, .(id, NO.N2O, CO2.1.3, NO.N2O, CO2.1.3)]
# 
...
summary(powerTransform(mydata[,log(NO.N2O)]+1))
```
...
### linearity
################################################################################
# boxTidwell(log(NO.N2O) ~ fertilization + precipitation + tillage, data=mydata)
...
```{r}
library(leaps)
leaps <-regsubsets(log(NO.N2O) ~ ( fertilization + precipitation + tillage), data=mydata, nbest=4)
...
```{r, echo=F}
# fert
results.fert <- W.MW.oneway(log(NO.N2O) ~ fertilization, mydata, n=my.n)
summary(results.fert)
# plot(results.fert)

# precipitation
results.rain <- W.MW.oneway(log(NO.N2O) ~ precipitation, mydata, n=my.n)
summary(results.rain)
# plot(results.rain)

# tillage
results.tillage <- W.MW.oneway(log(NO.N2O) ~ tillage, mydata, n=my.n)
...
```{r, echo=F}
# fert
# results.fert <- PToneway(log(NO.N2O) ~ fertilization, mydata, n=my.n)
# summary.oneway(results.fert)
# 
# # precipitation
# results.rain <- PToneway(log(NO.N2O) ~ precipitation, mydata, n=my.n)
# summary.oneway(results.rain)
# 
# # tillage
# results.till <- PToneway(log(NO.N2O) ~ tillage, mydata, n=my.n)

---
title: "CO2 differences (by event). Without transformation"
output: html_document
---

```{r, echo=F}
data[,.(
        mean.CO2 = mean(CO2, na.rm=T),
        sd.CO2 = sd(CO2, na.rm=T),
        se.CO2 = sd(CO2, na.rm=T)/sqrt(sum(!is.na(CO2))),
        count.CO2 = sum(!is.na(CO2))
...
par(mfrow=c(1,2))
with(data, boxplot(CO2 ~ fertilization + event, main = "fert ~ event"))
with(data, boxplot(CO2 ~ event + fertilization, main = "event ~ fert"))

with(data, boxplot(CO2 ~ precipitation + event, main = "rain ~ event"))
with(data, boxplot(CO2 ~ event + precipitation, main = "event ~ rain"))

with(data, boxplot(CO2 ~ tillage + event, main = "till ~ event"))
with(data, boxplot(CO2 ~ event + tillage, main = "event ~ till"))

with(data, boxplot(CO2 ~ incubation + event, main = "inc ~ event"))
with(data, boxplot(CO2 ~ event + incubation, main = "event ~ inc"))

# ![CO2 fluxes](auxiliary_plots_for_ANOVA_reports/boxplot_CO2.png)
...
# 2. "[,CO2]" --> "[,transformation]"
```

```{r}
fit <- lm(CO2 ~ fertilization + precipitation + tillage + event, data=data)
fit.aov <- aov(CO2 ~ fertilization + precipitation + tillage + event, data=data)
fit.aov.int <- aov(CO2 ~ (fertilization + precipitation + tillage + event)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(CO2 ~ precipitation + tillage + event, data=data)
fit2 <- lm(CO2 ~ fertilization + tillage + event, data=data)
fit3 <- lm(CO2 ~ fertilization + precipitation + event, data=data)
...
# visualization
                # library(HH)
interaction2wt(CO2 ~ (fertilization + precipitation + tillage + event)^2, data=data, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# outlier removed
myfit <- lm(CO2 ~ fertilization + precipitation + tillage, data=data[-c(68)])
gvmodel <- gvlma(myfit)
summary(gvmodel)
```
...
outlierTest(fit)
next.outlier <- lm(CO2 ~ fertilization + precipitation + tillage, data=data[-c(68)]) # only 17 is an outlier
outlierTest(next.outlier)
next.outlier <- lm(CO2 ~ fertilization + precipitation + tillage, data=data[-c(68,54)]) # only 17 is an outlier
outlierTest(next.outlier)


# identify outliers
myoutliers <- c(68,54)
data[myoutliers, .(id, NO, N2O, NH3, NO.N2O, NO.N2O.NH3, CO2)]
...
summary(powerTransform(data[,CO2]))
```
...
### linearity
################################################################################
# boxTidwell(CO2 ~ fertilization + precipitation + tillage, data=data)
...
<span style="color:red">
--> fertilization add no significant prediction power to the model
</span>
...
```{r}
library(leaps)
leaps <-regsubsets(CO2 ~ fertilization + precipitation + tillage, data=data, nbest=4)

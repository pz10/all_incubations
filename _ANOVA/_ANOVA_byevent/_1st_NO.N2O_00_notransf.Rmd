---
title: "1st stretch NO+N2O differences. Approaches: Non-parametric and parametric (no transformation)"
...
par(mfrow=c(1,4))
with(mydata, boxplot(NO.N2O ~ fertilization, main = "fertilization"))
with(mydata, boxplot(NO.N2O ~ precipitation, main = "precipitation"))
with(mydata, boxplot(NO.N2O ~ tillage, main = "tillage"))
with(mydata, boxplot(NO.N2O ~ incubation, main = "incubation"))
...
```{r}
fit <- lm(NO.N2O ~ fertilization + precipitation + tillage, data=mydata)
fit.aov <- aov(NO.N2O ~ fertilization + precipitation + tillage, data=mydata)
fit.aov.int <- aov(NO.N2O ~ (fertilization + precipitation + tillage)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lm(NO.N2O ~  precipitation + tillage, data=mydata)
fit2 <- lm(NO.N2O ~  fertilization + tillage, data=mydata)
fit3 <- lm(NO.N2O ~  fertilization + precipitation, data=mydata)

fit.int <- lm(NO.N2O ~ ( fertilization + precipitation + tillage)^2, data=mydata)

# fit <- lm(NO.N2O ~ fertilization + precipitation + tillage + fertilization:precipitation + fertilization:tillage, data=mydata)
# fit.aov <- fit.aov <- aov(NO.N2O ~ fertilization + precipitation + tillage + fertilization:precipitation + fertilization:tillage, data=mydata)
...
```{r}
fit.p <- lmp(NO.N2O ~  fertilization + precipitation + tillage, data=mydata)
fit.aovp <- aovp(NO.N2O ~  fertilization + precipitation + tillage, data=mydata)
fit.aovp.int <- aovp(NO.N2O ~ ( fertilization + precipitation + tillage)^2, data=mydata)

# factor removal; for later model comparison
fit1.p <- lmp(NO.N2O ~  precipitation + tillage, data=mydata)
fit2.p <- lmp(NO.N2O ~  fertilization + tillage, data=mydata)
fit3.p <- lmp(NO.N2O ~  fertilization + precipitation, data=mydata)

fit.int.p <- lmp(NO.N2O ~ ( fertilization + precipitation + tillage)^2, data=mydata)
...
# 
# # outlier removed
# myfit <- lm(NO.N2O ~ fertilization + precipitation + tillage, data=mydata[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(NO.N2O ~ fertilization + precipitation + tillage, data=mydata[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(NO.N2O ~ fertilization + precipitation + tillage, data=mydata[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(NO.N2O ~ fertilization + precipitation + tillage, data=mydata[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(NO.N2O ~ fertilization + precipitation + tillage, data=mydata[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
...
summary(powerTransform(mydata[,NO.N2O]+1))
```
...
### linearity
################################################################################
# boxTidwell(NO.N2O ~ fertilization + precipitation + tillage, data=mydata)
...
```{r}
library(leaps)
leaps <-regsubsets(NO.N2O ~ ( fertilization + precipitation + tillage), data=mydata, nbest=4)
...
```{r, echo=F}
# fert
results.fert <- W.MW.oneway(NO.N2O ~ fertilization, mydata, n=my.n)
summary(results.fert)
# plot(results.fert)

# precipitation
results.rain <- W.MW.oneway(NO.N2O ~ precipitation, mydata, n=my.n)
summary(results.rain)
# plot(results.rain)

# tillage
results.tillage <- W.MW.oneway(NO.N2O ~ tillage, mydata, n=my.n)
...
```{r, echo=F}
# fert
results.fert <- PToneway(NO.N2O ~ fertilization, mydata, n=my.n)
summary.oneway(results.fert)

# precipitation
results.rain <- PToneway(NO.N2O ~ precipitation, mydata, n=my.n)
summary.oneway(results.rain)

# tillage
results.till <- PToneway(NO.N2O ~ tillage, mydata, n=my.n)

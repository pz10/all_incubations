---
title: "2nd stretch N2O differences. Approaches: Non-parametric and parametric (no transformation)"
...
par(mfrow=c(1,4))
with(mydata, boxplot(N2O ~ fertilization, main = "fertilization"))
with(mydata, boxplot(N2O ~ precipitation, main = "precipitation"))
with(mydata, boxplot(N2O ~ tillage, main = "tillage"))
with(mydata, boxplot(N2O ~ incubation, main = "incubation"))
...
```{r}
fit <- lm(N2O ~ fertilization + precipitation + tillage, data=mydata)
fit.aov <- aov(N2O ~ fertilization + precipitation + tillage, data=mydata)
fit.aov.int <- aov(N2O ~ (fertilization + precipitation + tillage)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lm(N2O ~  precipitation + tillage, data=mydata)
fit2 <- lm(N2O ~  fertilization + tillage, data=mydata)
fit3 <- lm(N2O ~  fertilization + precipitation, data=mydata)

fit.int <- lm(N2O ~ ( fertilization + precipitation + tillage)^2, data=mydata)

# fit <- lm(N2O ~ fertilization + precipitation + tillage + fertilization:tillage, data=mydata)
# fit.aov <- fit.aov <- aov(N2O ~ fertilization + precipitation + tillage + fertilization:tillage, data=mydata)
...
```{r}
fit.p <- lmp(N2O ~  fertilization + precipitation + tillage, data=mydata)
fit.aovp <- aovp(N2O ~  fertilization + precipitation + tillage, data=mydata)
fit.aovp.int <- aovp(N2O ~ ( fertilization + precipitation + tillage)^2, data=mydata)

# factor removal; for later model comparison
fit1.p <- lmp(N2O ~  precipitation + tillage, data=mydata)
fit2.p <- lmp(N2O ~  fertilization + tillage, data=mydata)
fit3.p <- lmp(N2O ~  fertilization + precipitation, data=mydata)

fit.int.p <- lmp(N2O ~ ( fertilization + precipitation + tillage)^2, data=mydata)
...
# 
# # outlier removed
# myfit <- lm(N2O ~ fertilization + precipitation + tillage, data=mydata[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(N2O ~ fertilization + precipitation + tillage, data=mydata[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(N2O ~ fertilization + precipitation + tillage, data=mydata[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(N2O ~ fertilization + precipitation + tillage, data=mydata[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(N2O ~ fertilization + precipitation + tillage, data=mydata[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
...
summary(powerTransform(mydata[,N2O]))
```
...
### linearity
################################################################################
# boxTidwell(N2O ~ fertilization + precipitation + tillage, data=mydata)
...
```{r}
library(leaps)
leaps <-regsubsets(N2O ~ ( fertilization + precipitation + tillage), data=mydata, nbest=4)
...
```{r, echo=F}
# fert
results.fert <- W.MW.oneway(N2O ~ fertilization, mydata, n=my.n)
summary(results.fert)
# plot(results.fert)

# precipitation
results.rain <- W.MW.oneway(N2O ~ precipitation, mydata, n=my.n)
summary(results.rain)
# plot(results.rain)

# tillage
results.tillage <- W.MW.oneway(N2O ~ tillage, mydata, n=my.n)
...
```{r, echo=F}
# fert
results.fert <- PToneway(N2O ~ fertilization, mydata, n=my.n)
summary.oneway(results.fert)

# precipitation
results.rain <- PToneway(N2O ~ precipitation, mydata, n=my.n)
summary.oneway(results.rain)

# tillage
results.till <- PToneway(N2O ~ tillage, mydata, n=my.n)

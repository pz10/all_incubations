---
title: "3rd stretch N2O differences. Approaches: Non-parametric and parametric (log transformation)"
...
par(mfrow=c(1,4))
with(mydata, boxplot(log(N2O) ~ fertilization, main = "fertilization"))
with(mydata, boxplot(log(N2O) ~ precipitation, main = "precipitation"))
with(mydata, boxplot(log(N2O) ~ tillage, main = "tillage"))
with(mydata, boxplot(log(N2O) ~ incubation, main = "incubation"))


par(mfrow=c(1,4))
ggplot(data=mydata, aes(x=fertilization, y=N2O)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=mydata, aes(x=precipitation, y=N2O)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=mydata, aes(x=tillage, y=N2O)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=mydata, aes(x=incubation, y=N2O)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
...
```{r, echo=F}
# replace strings:
# 1. "log(N2O) ~" --> "transformation ~"
# 2. "[,log(N2O)]" --> "[,transformation]"
```

```{r}
fit <- lm(log(N2O) ~ fertilization + precipitation + tillage, data=mydata)
fit.aov <- aov(log(N2O) ~ fertilization + precipitation + tillage, data=mydata)
fit.aov.int <- aov(log(N2O) ~ (fertilization + precipitation + tillage)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lm(log(N2O) ~  precipitation + tillage, data=mydata)
fit2 <- lm(log(N2O) ~  fertilization + tillage, data=mydata)
fit3 <- lm(log(N2O) ~  fertilization + precipitation, data=mydata)

fit.int <- lm(log(N2O) ~ ( fertilization + precipitation + tillage)^2, data=mydata)

# fit <- lm(log(N2O) ~ fertilization + precipitation + tillage + fertilization:precipitation + fertilization:tillage, data=mydata)
# fit.aov <- fit.aov <- aov(log(N2O) ~ fertilization + precipitation + tillage + fertilization:precipitation + fertilization:tillage, data=mydata)
...
```{r}
fit.p <- lmp(log(N2O) ~  fertilization + precipitation + tillage, data=mydata)
fit.aovp <- aovp(log(N2O) ~  fertilization + precipitation + tillage, data=mydata)
fit.aovp.int <- aovp(log(N2O) ~ ( fertilization + precipitation + tillage)^2, data=mydata)

# factor removal; for later model comparison
fit1.p <- lmp(log(N2O) ~  precipitation + tillage, data=mydata)
fit2.p <- lmp(log(N2O) ~  fertilization + tillage, data=mydata)
fit3.p <- lmp(log(N2O) ~  fertilization + precipitation, data=mydata)

fit.int.p <- lmp(log(N2O) ~ ( fertilization + precipitation + tillage)^2, data=mydata)
...
# 
# # outlier removed
# myfit <- lm(log(N2O) ~ fertilization + precipitation + tillage, data=mydata[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(log(N2O) ~ fertilization + precipitation + tillage, data=mydata[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(log(N2O) ~ fertilization + precipitation + tillage, data=mydata[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(log(N2O) ~ fertilization + precipitation + tillage, data=mydata[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(log(N2O) ~ fertilization + precipitation + tillage, data=mydata[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
# myoutliers <- c(68,54)
# mydata[myoutliers, .(id, N2O, CO2.1.3, N2O, CO2.1.3)]
# 
# mydata[myoutliers, N2O]; fitted(fit)[myoutliers]
...
summary(powerTransform(mydata[,log(N2O)]+3))
```
...
### linearity
################################################################################
# boxTidwell(log(N2O) ~ fertilization + precipitation + tillage, data=mydata)
...
```{r}
library(leaps)
leaps <-regsubsets(log(N2O) ~ ( fertilization + precipitation + tillage), data=mydata, nbest=4)
...
```{r, echo=F}
# fert
results.fert <- W.MW.oneway(log(N2O) ~ fertilization, mydata, n=my.n)
summary(results.fert)
# plot(results.fert)

# precipitation
results.rain <- W.MW.oneway(log(N2O) ~ precipitation, mydata, n=my.n)
summary(results.rain)
# plot(results.rain)

# tillage
results.tillage <- W.MW.oneway(log(N2O) ~ tillage, mydata, n=my.n)
...
```{r, echo=F}
# fert
# results.fert <- PToneway(log(N2O) ~ fertilization, mydata, n=my.n)
# summary.oneway(results.fert)
# 
# # precipitation
# results.rain <- PToneway(log(N2O) ~ precipitation, mydata, n=my.n)
# summary.oneway(results.rain)
# 
# # tillage
# results.till <- PToneway(log(N2O) ~ tillage, mydata, n=my.n)

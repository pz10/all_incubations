---
title: "CO2 differences (2nd event). Sqrt transformation"
...
```{r}
fit <- lm(sqrt(CO2) ~ fertilization + precipitation + tillage, data=data)
fit.aov <- aov(sqrt(CO2) ~ fertilization + precipitation + tillage, data=data)
fit.aov.int <- aov(sqrt(CO2) ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(sqrt(CO2) ~ precipitation + tillage, data=data)
fit2 <- lm(sqrt(CO2) ~ fertilization + tillage, data=data)
fit3 <- lm(sqrt(CO2) ~ fertilization + precipitation, data=data)
...
# visualization
                # library(HH)
interaction2wt(sqrt(CO2) ~ (fertilization + precipitation + tillage)^2, data=data, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# outlier removed
myfit <- lm(sqrt(CO2) ~ fertilization + precipitation + tillage, data=data[-c(68)])
gvmodel <- gvlma(myfit)
summary(gvmodel)
```
...
outlierTest(fit)
next.outlier <- lm(sqrt(CO2) ~ fertilization + precipitation + tillage, data=data[-c(68)]) # only 17 is an outlier
outlierTest(next.outlier)
next.outlier <- lm(sqrt(CO2) ~ fertilization + precipitation + tillage, data=data[-c(68,54)]) # only 17 is an outlier
outlierTest(next.outlier)
...
summary(powerTransform(data[,sqrt(CO2)]))
```
...
### linearity
################################################################################
# boxTidwell(sqrt(CO2) ~ fertilization + precipitation + tillage, data=data)
...
```{r}
library(leaps)
leaps <-regsubsets(sqrt(CO2) ~ fertilization + precipitation + tillage, data=data, nbest=4)

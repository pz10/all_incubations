---
title: "1st stretch NO+N2O+NH3 differences. Approaches: Non-parametric and parametric (log transformation; remove tillage and include precipitation in the model)"
...
```{r, echo=F}
mydata <- copy(data)
mydata <- mydata[event=="1"]
mydata <- mydata[precipitation!="D"]
...
#         se.NO.N2O.NH3 = sd(NO.N2O.NH3, na.rm=T)/sqrt(sum(!is.na(NO.N2O.NH3))),
#         count.NO.N2O.NH3 = sum(!is.na(NO.N2O.NH3))
# ), by=.(incubation, fertilization, tillage)]
...
par(mfrow=c(1,4))
with(mydata, boxplot(log(NO.N2O.NH3) ~ incubation, main = "incubation"))
with(mydata, boxplot(log(NO.N2O.NH3) ~ fertilization, main = "fertilization"))
with(mydata, boxplot(log(NO.N2O.NH3) ~ tillage, main = "tillage"))
with(mydata, boxplot(log(NO.N2O.NH3) ~ precipitation, main = "precipitation"))
...
```{r, echo=F}
# replace strings:
# 1. "log(NO.N2O.NH3) ~" --> "transformation ~"
# 2. "[,log(NO.N2O.NH3)]" --> "[,transformation]"
```

```{r}
fit <- lm(log(NO.N2O.NH3) ~ incubation + fertilization + precipitation, data=mydata)
fit.aov <- aov(log(NO.N2O.NH3) ~ incubation + fertilization + precipitation, data=mydata)
fit.aov.int <- aov(log(NO.N2O.NH3) ~ (incubation + fertilization + precipitation)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lm(log(NO.N2O.NH3) ~  fertilization + precipitation, data=mydata)
fit2 <- lm(log(NO.N2O.NH3) ~  incubation + precipitation, data=mydata)
fit3 <- lm(log(NO.N2O.NH3) ~  incubation + fertilization, data=mydata)

fit.int <- lm(log(NO.N2O.NH3) ~ ( incubation + fertilization + precipitation)^2, data=mydata)

fit <- lm(log(NO.N2O.NH3) ~ incubation + fertilization + precipitation + incubation:fertilization + incubation:tillage, data=mydata)
fit.aov <- fit.aov <- aov(log(NO.N2O.NH3) ~ incubation + fertilization + precipitation + incubation:fertilization + incubation:tillage, data=mydata)
...
```{r}
fit.p <- lmp(log(NO.N2O.NH3) ~  incubation + fertilization + precipitation, data=mydata)
fit.aovp <- aovp(log(NO.N2O.NH3) ~  incubation + fertilization + precipitation, data=mydata)
fit.aovp.int <- aovp(log(NO.N2O.NH3) ~ ( incubation + fertilization + precipitation)^2, data=mydata)

# factor removal; for later model comparison
fit1.p <- lmp(log(NO.N2O.NH3) ~  fertilization + precipitation, data=mydata)
fit2.p <- lmp(log(NO.N2O.NH3) ~  incubation + precipitation, data=mydata)
fit3.p <- lmp(log(NO.N2O.NH3) ~  incubation + fertilization, data=mydata)

fit.int.p <- lmp(log(NO.N2O.NH3) ~ ( incubation + fertilization + precipitation)^2, data=mydata)
...
# 
# INDEPENDENCE OF ERRORS (autocorrelation) 
# independence (it might be of interest to check whether ther is any autocorrelation regarding chronological sampling order whitin one precipitation)
...
# 
# # outlier removed
# myfit <- lm(log(NO.N2O.NH3) ~ incubation + fertilization + precipitation, data=mydata[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(log(NO.N2O.NH3) ~ incubation + fertilization + precipitation, data=mydata[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(log(NO.N2O.NH3) ~ incubation + fertilization + precipitation, data=mydata[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(log(NO.N2O.NH3) ~ incubation + fertilization + precipitation, data=mydata[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(log(NO.N2O.NH3) ~ incubation + fertilization + precipitation, data=mydata[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
...
summary(powerTransform(mydata[,log(NO.N2O.NH3)]+1))
```
...
### linearity
################################################################################
# boxTidwell(log(NO.N2O.NH3) ~ incubation + fertilization + precipitation, data=mydata)
...
```{r}
library(leaps)
leaps <-regsubsets(log(NO.N2O.NH3) ~ ( incubation + fertilization + precipitation), data=mydata, nbest=4)
...
tuk <- glht(fit.aov, linfct=mcp(incubation="Tukey"))
...
```{r, echo=F}
# fert
# results.fert <- W.MW.oneway(log(NO.N2O.NH3) ~ incubation, mydata, n=my.n)
# summary(results.fert)
# # plot(results.fert)
# 
# # fertilization
# results.rain <- W.MW.oneway(log(NO.N2O.NH3) ~ fertilization, mydata, n=my.n)
# summary(results.rain)
# # plot(results.rain)
# 
# # # tillage
# # results.tillage <- W.MW.oneway(log(NO.N2O.NH3) ~ tillage, mydata, n=my.n)
...
```{r, echo=F}
# # fert
# results.fert <- PToneway(log(NO.N2O.NH3) ~ incubation, mydata, n=my.n)
# summary.oneway(results.fert)
# 
# # fertilization
# results.rain <- PToneway(log(NO.N2O.NH3) ~ fertilization, mydata, n=my.n)
# summary.oneway(results.rain)
# 
# # # tillage
# # results.till <- PToneway(log(NO.N2O.NH3) ~ tillage, mydata, n=my.n)

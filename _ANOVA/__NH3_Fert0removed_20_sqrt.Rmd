---
title: "NH3 (incubations A,B,C) differences. Fert=0 removed. Approaches: Non-parametric and parametric (SQRT transformation)"
...
data[,fertilization:= factor(fertilization)]

data[,.(
        mean.NH3 = mean(NH3, na.rm=T),
        sd.NH3 = sd(NH3, na.rm=T),
        se.NH3 = sd(NH3, na.rm=T)/sqrt(sum(!is.na(NH3))),
        count.NH3 = sum(!is.na(NH3))
), by=fertilization]
...
par(mfrow=c(1,4))
with(data, boxplot(NH3 ~ fertilization, main = "fertilization"))
with(data, boxplot(NH3 ~ precipitation, main = "precipitation"))
with(data, boxplot(NH3 ~ tillage, main = "tillage"))
with(data, boxplot(NH3 ~ incubation, main = "incubation"))


par(mfrow=c(1,4))
ggplot(data=data, aes(x=fertilization, y=NH3)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=data, aes(x=precipitation, y=NH3)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=data, aes(x=tillage, y=NH3)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=data, aes(x=incubation, y=NH3)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()

```
![NH3 fluxes](auxiliary_plots_for_ANOVA_reports/ABC_boxplot_NH3.png)
...
```{r}
fit <- lm(sqrt(NH3) ~ incubation + fertilization + precipitation + tillage, data=data)
fit.aov <- aov(sqrt(NH3) ~ incubation + fertilization + precipitation + tillage, data=data)
fit.aov.int <- aov(sqrt(NH3) ~ (incubation + fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(sqrt(NH3) ~ incubation + precipitation + tillage, data=data)
fit2 <- lm(sqrt(NH3) ~ incubation + fertilization + tillage, data=data)
fit3 <- lm(sqrt(NH3) ~ incubation + fertilization + precipitation, data=data)

fit.int <- lm(sqrt(NH3) ~ (incubation + fertilization + precipitation + tillage)^2, data=data)
...
```{r}
fit.p <- lmp(NH3 ~ incubation + fertilization + precipitation + tillage, data=data)
fit.aovp <- aovp(NH3 ~ incubation + fertilization + precipitation + tillage, data=data)
fit.aovp.int <- aovp(NH3 ~ (incubation + fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1.p <- lmp(NH3 ~ incubation + precipitation + tillage, data=data)
fit2.p <- lmp(NH3 ~ incubation + fertilization + tillage, data=data)
fit3.p <- lmp(NH3 ~ incubation + fertilization + precipitation, data=data)

fit.int.p <- lmp(NH3 ~ (incubation + fertilization + precipitation + tillage)^2, data=data)
...
# 
# # outlier removed
# myfit <- lm(NH3 ~ fertilization + precipitation + tillage, data=data[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(NH3 ~ fertilization + precipitation + tillage, data=data[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(NH3 ~ fertilization + precipitation + tillage, data=data[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(NH3 ~ fertilization + precipitation + tillage, data=data[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(NH3 ~ fertilization + precipitation + tillage, data=data[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
# myoutliers <- c(68,54)
# data[myoutliers, .(id, NH3, CO2.1.3, NH3, CO2.1.3)]
# 
...
summary(powerTransform(data[,sqrt(NH3)]))
```
...
### linearity
################################################################################
# boxTidwell(NH3 ~ fertilization + precipitation + tillage, data=data)
...
```{r}
library(leaps)
leaps <-regsubsets(sqrt(NH3) ~ (incubation + fertilization + precipitation + tillage), data=data, nbest=4)
...
```{r, echo=F}
# fert
results.fert <- W.MW.oneway(NH3 ~ fertilization, data, n=my.n)
summary(results.fert)
plot(results.fert)

# precipitation
results.rain <- W.MW.oneway(NH3 ~ precipitation, data, n=my.n)
summary(results.rain)
plot(results.rain)

# tillage
results.tillage <- W.MW.oneway(NH3 ~ tillage, data, n=my.n)
...
```{r, echo=F}
# fert
results.fert <- PToneway(NH3 ~ fertilization, data, n=my.n)
summary.oneway(results.fert)

# precipitation
results.rain <- PToneway(NH3 ~ precipitation, data, n=my.n)
summary.oneway(results.rain)

# tillage
results.till <- PToneway(NH3 ~ tillage, data, n=my.n)

---
title: "NH3 (incubations A,B,C) differences. Fert=0 removed. Approaches: Non-parametric and parametric (log transformation)"
...
data[,.(
        ...
        se.NH3 = sd(NH3, na.rm=T)/log(sum(!is.na(NH3))),
...
```{r}
fit <- lm(log(NH3) ~ incubation + fertilization + precipitation + tillage, data=data)
fit.aov <- aov(log(NH3) ~ incubation + fertilization + precipitation + tillage, data=data)
fit.aov.int <- aov(log(NH3) ~ (incubation + fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(log(NH3) ~ incubation + precipitation + tillage, data=data)
fit2 <- lm(log(NH3) ~ incubation + fertilization + tillage, data=data)
fit3 <- lm(log(NH3) ~ incubation + fertilization + precipitation, data=data)

fit.int <- lm(log(NH3) ~ (incubation + fertilization + precipitation + tillage)^2, data=data)
...
summary(powerTransform(data[,log(NH3+2)]))
```
...
```{r}
library(leaps)
leaps <-regsubsets(log(NH3) ~ (incubation + fertilization + precipitation + tillage), data=data, nbest=4)

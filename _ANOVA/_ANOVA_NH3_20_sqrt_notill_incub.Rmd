---
title: "NH3 differences. sqrt(NH3) transformation"
...
```{r}
fit <- lm(sqrt(NH3) ~ incubation + fertilization + precipitation, data=mydata)
fit.aov <- aov(sqrt(NH3) ~ incubation + fertilization + precipitation, data=mydata)
fit.aov.int <- aov(sqrt(NH3) ~ (incubation + fertilization + precipitation)^2, data=mydata)

# factor removal; for later model comparison
fit1 <- lm(sqrt(NH3) ~ incubation + precipitation, data=mydata)
fit2 <- lm(sqrt(NH3) ~ incubation + fertilization, data=mydata)
fit3 <- lm(sqrt(NH3) ~ fertilization + precipitation, data=mydata)
...
# visualization
                # library(HH)
interaction2wt(sqrt(NH3) ~ (incubation + fertilization + precipitation)^2, data=mydata, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# # outlier removed
# myfit <- lm(sqrt(NH3) ~ incubation + fertilization + precipitation, data=mydata[-c(68)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
```
...
summary(powerTransform(mydata[,sqrt(NH3)+1]))
```
...
### linearity
################################################################################
# boxTidwell(sqrt(NH3) ~ incubation + fertilization + precipitation, data=mydata)
...
<span style="color:red">
--> These 3-factors add prediction power to the model 
</span>
...
```{r}
library(leaps)
leaps <-regsubsets(sqrt(NH3) ~ incubation + fertilization + precipitation, data=mydata, nbest=4)

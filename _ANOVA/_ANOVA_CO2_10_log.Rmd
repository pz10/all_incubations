---
title: "CO2 differences. Log transformation"
...
par(mfrow=c(1,4))
with(data, boxplot(CO2 ~ fertilization, main = "fertilization"))
with(data, boxplot(CO2 ~ precipitation, main = "precipitation"))
with(data, boxplot(CO2 ~ tillage, main = "tillage"))
with(data, boxplot(CO2 ~ incubation, main = "incubation"))
...
```{r}
fit <- lm(log(CO2) ~ fertilization + precipitation + tillage, data=data)
fit.aov <- aov(log(CO2) ~ fertilization + precipitation + tillage, data=data)
fit.aov.int <- aov(log(CO2) ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(log(CO2) ~ precipitation + tillage, data=data)
fit2 <- lm(log(CO2) ~ fertilization + tillage, data=data)
fit3 <- lm(log(CO2) ~ fertilization + precipitation, data=data)
...
# visualization
                # library(HH)
interaction2wt(log(CO2) ~ (fertilization + precipitation + tillage)^2, data=data, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
# outlier removed
myfit <- lm(log(CO2) ~ fertilization + precipitation + tillage, data=data[-c(68)])
gvmodel <- gvlma(myfit)
summary(gvmodel)
```
...
outlierTest(fit)
next.outlier <- lm(log(CO2) ~ fertilization + precipitation + tillage, data=data[-c(68)]) # only 17 is an outlier
outlierTest(next.outlier)
next.outlier <- lm(log(CO2) ~ fertilization + precipitation + tillage, data=data[-c(68,54)]) # only 17 is an outlier
outlierTest(next.outlier)
...
summary(powerTransform(data[,log(CO2)]))
```
...
### linearity
################################################################################
# boxTidwell(log(CO2) ~ fertilization + precipitation + tillage, data=data)
...
```{r}
library(leaps)
leaps <-regsubsets(log(CO2) ~ fertilization + precipitation + tillage, data=data, nbest=4)

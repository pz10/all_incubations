---
title: "NO+N2O differences. log(NO +N2O) transformation"
...
log transformation seems to fit; tillage add no prediction power to the OLS model
</span>
...
# visualization
                # library(HH)
interaction2wt(log(NO.N2O) ~ (fertilization + precipitation + tillage)^2, data=data, main=paste("main effect and 2-way interactions \n",fit.aov.int$cal[2]))
```
...
outlierTest(fit)
next.outlier <- lm(log(NO.N2O) ~ fertilization + precipitation + tillage, data=data[-c(50)]) 
...
summary(powerTransform(data[,log(NO.N2O)]))
```
...
<span style="color:red">
--> 2 factorial model (without tillage) has the lowest AIC
</span>
